...
const AbstractTable: React.FC<{
  title: string;
  data: Record<Category, AbstractData>;
  unit: string;
  decimals: number;
}> = ({ title, data, unit, decimals }) => {
  const total = {
    opening:
      data.balvatika.opening + data.primary.opening + data.middle.opening,
    received:
      data.balvatika.received + data.primary.received + data.middle.received,
    total: data.balvatika.total + data.primary.total + data.middle.total,
    consumed:
      (data.balvatika.consumed || 0) +
      (data.primary.consumed || 0) +
      (data.middle.consumed || 0),
    expenditure:
      (data.balvatika.expenditure || 0) +
      (data.primary.expenditure || 0) +
      (data.middle.expenditure || 0),
    balance:
      data.balvatika.balance + data.primary.balance + data.middle.balance,
  };
  const isCash = unit === "₹";
  ...
  const thClasses =
    "p-2 border border-slate-200 dark:border-slate-800 whitespace-nowrap";
  const tdClasses =
    "p-2 border border-slate-200 dark:border-slate-800 whitespace-nowrap";
        ...
        <table className="min-w-[540px] w-full text-xs text-left border-collapse">
            ...
            {categories.map((cat) => (
              <tr key={cat}>
                ...
                <td className={`${tdClasses} text-right`}>
                  {data[cat].opening.toFixed(decimals)}
                </td>
                <td className={`${tdClasses} text-right`}>
                  {data[cat].received.toFixed(decimals)}
                </td>
                <td className={`${tdClasses} text-right`}>
                  {data[cat].total.toFixed(decimals)}
                </td>
                <td className={`${tdClasses} text-right`}>
                  {data[cat][key]?.toFixed(decimals)}
                </td>
                <td className={`${tdClasses} text-right font-semibold`}>
                  {data[cat].balance.toFixed(decimals)}
                </td>
              </tr>
            ))}
          </tbody>
          <tfoot className="font-semibold bg-slate-100 dark:bg-slate-900">
            <tr>
              ...
              <td className={`${tdClasses} text-right`}>
                {total.opening.toFixed(decimals)}
              </td>
              <td className={`${tdClasses} text-right`}>
                {total.received.toFixed(decimals)}
              </td>
              <td className={`${tdClasses} text-right`}>
                {total.total.toFixed(decimals)}
              </td>
              <td className={`${tdClasses} text-right`}>
                {total[key].toFixed(decimals)}
              </td>
              <td className={`${tdClasses} text-right`}>
                {total.balance.toFixed(decimals)}
              </td>
            </tr>
          </tfoot>
        </table>
...
const CategoryAbstractTable: React.FC<{
  title: string;
  data: AbstractData;
  unit: string;
  decimals: number;
  ...
  const key = isCash ? "expenditure" : "consumed";
  const thClasses =
    "p-2 border border-slate-200 dark:border-slate-800 whitespace-nowrap text-left";
  const tdClasses =
    "p-2 border border-slate-200 dark:border-slate-800 whitespace-nowrap text-right";
          ...
          <tbody>
            <tr className="bg-white dark:bg-slate-950">
              ...
              <td className={tdClasses}>
                {(data[key] || 0).toFixed(decimals)}
              </td>
              <td
                className={`${tdClasses} font-semibold text-indigo-600 dark:text-indigo-400`}
              >
                {data.balance.toFixed(decimals)}
              </td>
            </tr>
          </tbody>
        </table>
  ...
  <div className="overflow-x-auto max-h-72 rounded-xl border border-slate-200 dark:border-slate-800">
    <table className="w-full text-xs text-left">
      <thead className="bg-slate-50 dark:bg-slate-900/50 sticky top-0 z-10 shadow-sm">
        <tr>
          <th className="p-3 font-medium text-slate-500 dark:text-slate-400">
            Date
          </th>
          <th className="p-3 font-medium text-slate-500 dark:text-slate-400 text-right">
            Present
          </th>
          <th className="p-3 font-medium text-slate-500 dark:text-slate-400 text-right">
            Rice (kg)
        ...
        {entries.length > 0 ? (
          [...entries].reverse().map((entry) => {
            ...
            return (
              <tr
                key={entry.id}
                className={`border-b border-slate-100 dark:border-slate-800/50 last:border-0 ${isNoMealDay ? "bg-red-50/50 dark:bg-red-900/10" : "bg-white dark:bg-slate-950"}`}
              >
                <td className="p-3">{date.toLocaleDateString("en-IN")}</td>
                {isNoMealDay ? (
                  <td
                    colSpan={3}
                    className="p-3 text-center text-red-600 dark:text-red-400 text-xs italic"
                  >
          ...
          <tr className="border-b border-slate-100 dark:border-slate-800/50">
            <td
              colSpan={4}
              className="p-4 text-center text-slate-500 dark:text-slate-400 bg-white dark:bg-slate-950"
            >
              No entries with meals served for this category in the selected
              month.
            </td>
          </tr>
        )}
    ...
    <div className="overflow-x-auto rounded-xl border border-slate-200 dark:border-slate-800">
      <table className="min-w-[768px] w-full text-xs text-left border-collapse">
        <thead className="bg-slate-50 dark:bg-slate-900/50 sticky top-0 z-10 shadow-sm">
          <tr>
            <th
              className={`${thClasses} font-medium text-slate-500 dark:text-slate-400`}
            >
              S.No
            </th>
            <th
            ...
            <th
              className={`${thClasses} font-medium text-slate-500 dark:text-slate-400 text-right`}
            >
              Roll
            </th>
            <th
        ...
        <tbody>
          {dailyData.map((day) => {
            ...
            const reasonText = day.isSunday
              ? "Sunday"
              : day.reason || "No Meal Served";
            ...
            return (
              ...
              >
                {isNoMealDay ? (
                  <td
                    colSpan={11}
                    className={`${tdClasses} text-center italic text-red-600 dark:text-red-400`}
                  >
                  ...
                  <>
                    ...
                    <td className={`${tdClasses} text-right`}>
                      {day.riceUsed.toFixed(3)}
                    </td>
                    <td className={`${tdClasses} text-right`}>
                      {day.dalVeg.toFixed(2)}
                    </td>
                    <td className={`${tdClasses} text-right`}>
                      {day.oilCond.toFixed(2)}
                    </td>
                    <td className={`${tdClasses} text-right`}>
                      {day.salt.toFixed(2)}
                    </td>
        ...
        <tfoot className="font-semibold bg-slate-100 dark:bg-slate-900">
          <tr>
            <td colSpan={3} className={`${tdClasses} text-right`}>
              Total
            </td>
            <td className={`${tdClasses} text-right`}>{totals.present}</td>
            <td className={`${tdClasses} text-right`}>
              {totals.riceUsed.toFixed(3)}
            </td>
            <td className={`${tdClasses} text-right`}>
...
const MonthlySummary: React.FC = () => {
  ...
  const onRoll = useMemo(() => {
    ...
    settings.classRolls.forEach((c) => {
      const classTotal =
        c.general.boys + c.general.girls + c.stsc.boys + c.stsc.girls;
  ...
  const lastSavedBalanceRef = React.useRef<{
    month: string;
    balance: string;
  } | null>(null);
  ...
  useEffect(() => {
    if (closingBalance && monthEntries.length > 0) {
      ...
      const currentBalanceStr = closingBalanceString;

      if (
        !lastSavedBalanceRef.current ||
        lastSavedBalanceRef.current.month !== currentMonth ||
        lastSavedBalanceRef.current.balance !== currentBalanceStr
      ) {
        // Update ref FIRST to prevent re-entry if a re-render is triggered synchronously
        lastSavedBalanceRef.current = {
          month: currentMonth,
          balance: currentBalanceStr,
        };

        console.log("MonthlySummary: Saving balance for", currentMonth);
        saveMonthlyBalance(currentMonth, closingBalance);
      }
    }
      ...
      {monthEntries.length === 0 ? (
        ...
        <>
          {view === "overall" ? (
            ...
            <Card
              title={`${view.charAt(0).toUpperCase() + view.slice(1)} Summary`}
            >
              <div className="flex flex-col gap-4">
                <CategoryAbstractTable
                  title="Rice Abstract"
                  data={riceAbstracts[view]}
                  unit="kg"
                  decimals={3}
                />
                <CategoryAbstractTable
                  title="Cash Abstract"
                  data={cashAbstracts[view]}
                  unit="₹"
                  decimals={2}
                />
              </div>
          ...
          <Card title="Meal & Attendance Totals">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-center">
              ...
              <div>
                ...
                <p className="text-lg font-semibold text-indigo-600 dark:text-indigo-400">
                  ₹{displayedTotals.expenditure.toFixed(2)}
                </p>
              </div>
              <div>
                <p className="text-xs font-medium text-slate-500 dark:text-slate-400">
                ...
                <p className="text-lg font-semibold text-indigo-600 dark:text-indigo-400">
                  {displayedTotals.rice.toFixed(3)} kg
                </p>
              </div>
            </div>
          </Card>
