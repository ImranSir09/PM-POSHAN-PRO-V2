...
export const AccordionItem: React.FC<{ id: string; title: ReactNode; children: ReactNode }> = ({ id, title, children }) => {
    ...
    return (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden transition-all duration-300 shadow-sm">
            <button
                onClick={() => toggleItem(id)}
                className="w-full flex justify-between items-center p-5 text-left font-semibold text-slate-800 dark:text-slate-100 text-base"
                aria-expanded={isOpen}
                aria-controls={`content-${id}`}
            >
                <span className="flex-1">{title}</span>
                <svg xmlns="http://www.w3.org/2000/svg" className={`h-5 w-5 text-slate-400 transition-transform transform ${isOpen ? 'rotate-180' : ''}`} viewBox="0 0 20 20" fill="currentColor">
            ...
            >
                <div className="p-5 pt-2 border-t border-slate-100 dark:border-slate-800/50">
                    {children}
                </div>
            </div>
        </div>
